...
- Workflows in `.github/workflows/`: tests (Python 3.11–3.13, Ubuntu/macOS/Windows, including `v*` tags). Pushing a `v*` tag runs tests, creates a GitHub Release, and publishes to PyPI; conda and citation update chain off that successful tag publish. `workflow_dispatch` on PyPI Publish only bumps the version and pushes the tag (uses `GH_PAT` so the tag event can trigger the publish job).
