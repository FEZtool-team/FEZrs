...
- The single source of version truth is `.bumpversion.cfg`; `setup.py` parses it, and the conda recipe (`recip/meta.yaml`) reads `FEZRS_VERSION` from the environment. Tags are `v{version}` (`tag_name = v{new_version}`).
...
- Workflows in `.github/workflows/`: tests (Python 3.11–3.13, Ubuntu/macOS/Windows, including `v*` tags). Pushing a `v*` tag, or running **PyPI Publish** via `workflow_dispatch`, creates the GitHub Release and publishes to PyPI (checkout uses `GITHUB_TOKEN`; do not pass `secrets.GH_PAT` into `actions/checkout`). Conda and citation update chain off a successful PyPI publish (`push` of a `v*` tag or `workflow_dispatch`).
